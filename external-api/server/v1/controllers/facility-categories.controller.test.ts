...
describe('findAll', () => {
  ...
  it(`should return ${HttpStatusCode.InternalServerError} when an error is thrown`, async () => {
    ...
    expect(console.error).toHaveBeenNthCalledWith(
      2,
      '🚩 Error occurred during facility categories endpoint call %o',
    ...
    expect(mockResponse._getData()).toEqual({ status: HttpStatusCode.InternalServerError, message: 'Error occurred during facility categories endpoint call' });
  });
  ...
  it(`should return ${HttpStatusCode.Ok}`, async () => {
    ...
    expect(axios).toHaveBeenLastCalledWith({
      method: 'get',
      url: `${APIM_MDM_URL}v2/ods/facility-categories`,
      headers,
    });
