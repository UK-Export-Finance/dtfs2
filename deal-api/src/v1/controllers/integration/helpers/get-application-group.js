...
const getApplicationGroup = (deal) => {
  const bssCount = deal.bondTransactions.items
    .filter((b) => b.status === CONSTANTS.FACILITIES.STATUS.COMPLETED).length;
  const ewcsCount = deal.loanTransactions.items
    .filter((l) => l.status === CONSTANTS.FACILITIES.STATUS.COMPLETED).length;
