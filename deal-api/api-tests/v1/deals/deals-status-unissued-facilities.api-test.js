...
const completedDealUnissuedAndUnissuedFacilities = require('../../fixtures/deal-fully-completed-issued-and-unissued-facilities');
...
describe('/v1/deals/:id/status - unissued facilities', () => {
  let noRoles;
  let aBarclaysMaker;
  ...
  describe('PUT /v1/deals/:id/status', () => {
      ...
      describe('when a deal contains bonds with an `Issued` bondStage that do NOT have a requestedCoverStartDate', () => {
        it('should add todays date to such bonds', async () => {
          ...
          const newDealWithBonds = {
            ...completedDealUnissuedAndUnissuedFacilities,
            bondTransactions: {
              items: [
                {
                  ...baseBond,
    ...
    describe('when the deal status changes to `Further Maker\'s input required`', () => {
      let createdDeal;
      let updatedDeal;
    ...
    describe('when the deal status changes to `Submitted`', () => {
      let createdDeal;
      let updatedDeal;
      ...
      // TODO: add loans
      describe('any bonds that have details provided, but not yet been submitted', () => {
