...
describe('/v1/deals/:id/eligibility-criteria', () => {
  ...
  describe('PUT /v1/deals/:id/eligibility-criteria', () => {
    it('401s requests that do not present a valid Authorization token', async () => {
      const {status} = await put(updatedEC).to(`/v1/deals/123456789012/eligibility-criteria`);

      expect(status).toEqual(401);
    });

    it('401s requests that do not come from a user with role=maker', async () => {
      const {status} = await put(updatedEC, aUserWithoutRoles).to(`/v1/deals/123456789012/eligibility-criteria`);
    ...
    it('401s requests if <user>.bank != <resource>/details.owningBank', async () => {
      ...
      const {status} = await put(updatedEC, user2).to(`/v1/deals/${newId}/eligibility-criteria`);

      expect(status).toEqual(401);
    });

    it('404s requests for unknown resources', async () => {
      const {status} = await put(updatedEC, user1).to(`/v1/deals/123456789012/eligibility-criteria`);
    ...
    it('accepts requests if <user>.bank.id == *', async () => {
      ...
      const {status} = await put(updatedEC, superuser).to(`/v1/deals/${newId}/eligibility-criteria`);
    ...
    it('updates the eligibility criteria', async () => {
      ...
      await put(updatedEC, user1).to(`/v1/deals/${newId}/eligibility-criteria`);
