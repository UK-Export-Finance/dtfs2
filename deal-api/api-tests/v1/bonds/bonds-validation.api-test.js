...
describe('/v1/deals/:id/bond', () => {
  ...
  const updateBondInDeal = async (theDealId, bond) => {
    const bla = await as(aBarclaysMaker).put(bond).to(`/v1/deals/${theDealId}/bond/${bondId}`);
    return bla.body;
  };
  ...
  */
  ...
  describe('PUT /v1/deals/:id/bond/:bondId', () => {
    ...
    it('returns 400 with validation errors', async () => {
      ...
      const bondResponse = await as(aBarclaysMaker).put({}).to(`/v1/deals/${dealId}/bond/create`);
      const { bondId } = bondResponse.body;

      const { status, body } = await as(aBarclaysMaker).put({}).to(`/v1/deals/${dealId}/bond/${bondId}`);
    ...
    describe('when a bond has req.body.bondStage as `Issued`', () => {
      describe('when the requestedCoverStartDate has a value of 3 months or more', () => {
        it('should return requestedCoverStartDate validationError', async () => {
          ...
          const coverEndDate = moment(date).add(4, 'months');
          ...
          const body = await updateBondInDeal(dealId, updatedBond);
