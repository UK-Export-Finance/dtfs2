...
router.get(
  ...
  [validateToken, validateBank, validateRole({ role: [MAKER] })],
  (req, res) => changeUnissuedFacility(req, res),
);
// post change unissued facility to issued from facility list
router.post(
  '/application-details/:dealId/unissued-facilities/:facilityId/about',
  [validateToken, validateBank, validateRole({ role: [MAKER] })],
  (req, res) => postChangeUnissuedFacility(req, res),
);
...
router.get(
  ...
  [validateToken, validateBank, validateRole({ role: [MAKER] })],
  (req, res) => changeUnissuedFacilityPreview(req, res),
);
// post change unissued facilities to issued from application preview
router.post(
  '/application-details/:dealId/unissued-facilities/:facilityId/change',
  [validateToken, validateBank, validateRole({ role: [MAKER] })],
  (req, res) => postChangeUnissuedFacilityPreview(req, res),
);
