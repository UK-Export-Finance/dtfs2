...
describe('validation()', () => {
  ...
  describe.each([0, 1])('with GEF_DEAL_VERSION = %s', (dealVersion) => {
    ...
    it('should return object with errors populated if all dates are blank', async () => {
      ...
      if (dealVersion >= 1) {
        expectedErrors.errorSummary.push({ href: '#isUsingFacilityEndDate', text: 'Select if there is an end date for this facility' });
        expectedErrors.fieldErrors.isUsingFacilityEndDate = { text: 'Select if there is an end date for this facility' };
        expectedFacilityErrors.push({
    ...
    it('should return object with no errors if MIN and issue date 3 months from MIN submission date but 3 months from minSubmissionDate', async () => {
      api.getApplication.mockResolvedValue({
        submissionDate: `${getUnixTime(oneMonthAgo)}608`,
        manualInclusionNoticeSubmissionDate: `${getUnixTime(now)}608`,
      });
      mockRequest.body.facilityType = CONSTANTS.FACILITY_TYPE.CASH;
      mockRequest.body.facilityName = 'UKEF123';
      mockRequest.body.shouldCoverStartOnSubmission = 'false';
