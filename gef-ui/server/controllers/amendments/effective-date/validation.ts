 ...
 */
export const validateAndParseEffectiveFrom = (dayMonthYear: DayMonthYearInput, coverStartDate: Date): ErrorsOrValue<UnixTimestampSeconds> => {
  const errRef = 'effectiveFrom';
  ...
  const effectiveFrom = formattingErrorsOrDate.parsedDate;
  const now = startOfDay(new Date());
  const earliestEffectiveFromDate = sub(now, { days: AMENDMENT_MAXIMUM_EFFECTIVE_FROM_DAYS_IN_PAST });
  const latestEffectiveFromDate = add(now, { days: AMENDMENT_MAXIMUM_EFFECTIVE_FROM_DAYS_IN_FUTURE });

  if (isBefore(effectiveFrom, coverStartDate)) {
    return {
      errors: [
        {
          errRef,
  ...
  if (isBefore(effectiveFrom, earliestEffectiveFromDate)) {
    return {
      errors: [
        {
          errRef,
  ...
  if (isAfter(effectiveFrom, latestEffectiveFromDate)) {
    return {
      errors: [
        {
          errRef,
  ...
  return { value: getUnixTime(effectiveFrom) };
};
