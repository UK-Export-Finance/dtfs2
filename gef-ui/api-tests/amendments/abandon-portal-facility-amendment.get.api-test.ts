...
const url = `/application-details/${dealId}/facilities/${facilityId}/amendments/${amendmentId}/${PORTAL_AMENDMENT_PAGES.ABANDON}`;
...
describe(`GET ${url}`, () => {
  ...
  describe('when FF_PORTAL_FACILITY_AMENDMENTS_ENABLED is enabled', () => {
    ...
    it('should render `Confirm that you want to abandon` page', async () => {
      // Act
      const response = await getWithSessionCookie(sessionCookie);

      // Assert
      expect(response.status).toEqual(HttpStatusCode.Ok);
      expect(response.text).toContain('Confirm that you want to abandon');
    });
