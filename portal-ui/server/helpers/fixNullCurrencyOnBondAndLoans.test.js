...
const supportContractCurrency = { id: 'USD', text: 'US Dollar' };
const dealId = '12345';
...
describe('fixNullCurrencyOnBondAndLoans', () => {
  ...
  describe('when no loans or bonds have null currency where currencySameAsSupplyContractCurrency is true', () => {
    it('should not call postToApi', async () => {
      const bonds = [{ _id: '1', currencySameAsSupplyContractCurrency: 'true', currency: supportContractCurrency }];
      const loans = [{ _id: '3', currencySameAsSupplyContractCurrency: 'true', currency: supportContractCurrency }];

      await fixNullCurrencyOnBondAndLoans(bonds, loans, supportContractCurrency, dealId, userToken);
  ...
  describe('when loans and bonds have null currency where currencySameAsSupplyContractCurrency is true', () => {
    it('should call postToApi for each loan and bond with null currency where currencySameAsSupplyContractCurrency is true', async () => {
      const bonds = [
        ...
        { _id: '2', currencySameAsSupplyContractCurrency: 'true', currency: supportContractCurrency },
      ];
      const loans = [
        { _id: '3', currencySameAsSupplyContractCurrency: 'true', currency: null },
        { _id: '4', currencySameAsSupplyContractCurrency: 'true', currency: supportContractCurrency },
      ];

      await fixNullCurrencyOnBondAndLoans(bonds, loans, supportContractCurrency, dealId, userToken);

      expect(mockPostToApi).toHaveBeenCalledTimes(2);
      expect(mockPostToApi).toHaveBeenCalledWith(api.updateBond(dealId, bonds[0]._id, { currency: supportContractCurrency }, userToken));
      expect(mockPostToApi).toHaveBeenCalledWith(api.updateLoan(dealId, loans[0]._id, { currency: supportContractCurrency }, userToken));
    });
  });
  ...
  describe('when there are multiple loans and bonds with null currency where currencySameAsSupplyContractCurrency is true', () => {
    it('should call postToApi for each loan and bond with null currency where currencySameAsSupplyContractCurrency is true', async () => {
      ...
      expect(mockPostToApi).toHaveBeenCalledWith(api.updateBond(dealId, bonds[0]._id, { currency: supportContractCurrency }, userToken));
      expect(mockPostToApi).toHaveBeenCalledWith(api.updateBond(dealId, bonds[1]._id, { currency: supportContractCurrency }, userToken));
      expect(mockPostToApi).toHaveBeenCalledWith(api.updateLoan(dealId, loans[0]._id, { currency: supportContractCurrency }, userToken));
      expect(mockPostToApi).toHaveBeenCalledWith(api.updateLoan(dealId, loans[1]._id, { currency: supportContractCurrency }, userToken));
    });
  });

  describe('when there are no loans or bonds', () => {
    it('should not call postToApi', async () => {
      await fixNullCurrencyOnBondAndLoans([], [], supportContractCurrency, dealId, userToken);
