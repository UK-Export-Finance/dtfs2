...
describe('GET /login/new-access-code', () => {
  ...
  withPartial2faAuthValidationApiTests({
    makeRequestWithHeaders: (headers?: RequestHeaders) => get('/login/new-access-code', {}, headers),
  ...
  describe('page rendering', () => {
    ...
    it(`should render the new access code page with HTTP status ${HttpStatusCode.Ok}`, async () => {
      const response = await get('/login/new-access-code', {}, { Cookie: sessionCookie });

      expect(response.status).toEqual(HttpStatusCode.Ok);
      expect(response.text).toContain('New access code');
    });
    ...
    it('should redirect to /not-found when numberOfSignInOtpAttemptsRemaining is not 1', async () => {
      ...
      const response = await get('/login/new-access-code', {}, { Cookie: invalidSessionCookie });
