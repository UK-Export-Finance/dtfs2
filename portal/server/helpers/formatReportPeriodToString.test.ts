...
import { formatReportPeriodToString } from "./formatReportPeriodToString";


describe('formatReportPeriodToString', () => {
  ...
  it.each(testData)('returns $expectedResponse when $description', ({ reportPeriod, expectedResponse }) => {
    const response = formatReportPeriodToString(reportPeriod);
