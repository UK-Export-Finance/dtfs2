...
router.post('/login', async (req, res) => {
  ...
  try {
    ...
    try {
      ...
      console.error('Failed to send sign in link. The login flow will continue as the user can retry on the next page. The error was: %O', sendSignInLinkError);
    }
    return res.redirect('/login/check-your-email');
  } catch (loginError) {
    console.error('Failed to login: %O', loginError);
