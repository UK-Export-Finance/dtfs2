...
describe('generateTotalFeesAccruedError', () => {
  ...
  it('returns an error when the value is not a number', async () => {
    ...
    const expectedError = {
      errorMessage: 'Fees accrued since last report must be a number with a maximum of two decimal places',
      column: 1,
      row: 1,
      value: 'abc',
      exporter: testExporterName,
  ...
  it('returns an error when the value is too long', async () => {
    ...
    const expectedError = {
      errorMessage: `Fees accrued since last report must be ${FILE_UPLOAD.MAX_CELL_CHARACTER_COUNT} characters or less`,
      column: 1,
      row: 1,
      value: '1473812445951826593.52',
      exporter: testExporterName,
