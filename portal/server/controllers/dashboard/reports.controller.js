...
exports.getPortalReports = async (req, res) => {
  ...
  const dealWithConditions =
    (await api.getUkefDecisionReport(userToken, { ukefDecision: CONSTANTS.STATUS.UKEF_APPROVED_WITH_CONDITIONS })) ||
    [];
  const dealWithoutConditions =
    (await api.getUkefDecisionReport(userToken, { ukefDecision: CONSTANTS.STATUS.UKEF_APPROVED_WITHOUT_CONDITIONS })) ||
    [];
...
exports.downloadUnconditionalDecisionReport = async (req, res) => {
  const { userToken } = req.session;
  const { deals, columns } =
    (await downloadUkefDecisionReport(userToken, {
      ukefDecision: CONSTANTS.STATUS.UKEF_APPROVED_WITHOUT_CONDITIONS,
    })) || [];
...
exports.downloadConditionalDecisionReport = async (req, res) => {
  const { userToken } = req.session;
  const { deals, columns } =
    (await downloadUkefDecisionReport(userToken, { ukefDecision: CONSTANTS.STATUS.UKEF_APPROVED_WITH_CONDITIONS })) ||
    [];
