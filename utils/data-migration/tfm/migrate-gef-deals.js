...
const { datafixes, datafixTfmDealGef, datafixesTfmFacilitiesGef } = require('../helpers/datafixes');
...
/**
  ...
  */
const migrate = () => {
  console.info('\n\x1b[33m%s\x1b[0m', `🚀 Initiating ${CONSTANTS.DEAL.DEAL_TYPE.GEF} TFM migration v${version}.`, '\n\n');

  deals()
    .then((d) => datafixes(d))
    .then((d) => tfm(d))
    .then((d) => datafixTfmDealGef(d))
    .then((d) => datafixesTfmFacilitiesGef(d))
    .then(() => disconnect())
    .then(() => process.exit(1))
    .catch((error) => {
