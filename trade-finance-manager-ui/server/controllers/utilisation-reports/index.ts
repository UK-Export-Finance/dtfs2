...
import { InvalidUpdateReportStatusRequestError } from '../../errors/invalid-update-report-status-request.error';
  ...
  try {
    ...
    const reportsWithStatus = reportIdentifiers.map((reportIdentifier): ReportWithStatus => {
      ...
      if ('bankId' in reportIdentifier && statusToSet === UTILISATION_REPORT_RECONCILIATION_STATUS.REPORT_NOT_RECEIVED) {
        throw new InvalidUpdateReportStatusRequestError("Cannot set non-existing report to 'REPORT_NOT_RECEIVED'");
      }
    ...
    if (error instanceof InvalidUpdateReportStatusRequestError) {
