...
describe('controllers/utilisation-reports/add-payment', () => {
  ...
  describe('when navigating from premium payments table', () => {
    ...
    it('should render add payment page', async () => {
      ...
      jest.mocked(api.getSelectedFeeRecordsDetailsWithoutExistingCompatiblePaymentGroups).mockResolvedValue(aSelectedFeeRecordsDetails());
    ...
    it('should fetch and map selected fee record details', async () => {
      ...
      jest.mocked(api.getSelectedFeeRecordsDetailsWithoutExistingCompatiblePaymentGroups).mockResolvedValue(selectedFeeRecordDetailsResponse);

      // Act
      await addPayment(req, res);

      // Assert
      expect(api.getSelectedFeeRecordsDetailsWithoutExistingCompatiblePaymentGroups).toHaveBeenCalledWith('123', [456], requestSession.userToken);
      expect(res._getRenderView()).toEqual('utilisation-reports/add-payment.njk');
    ...
    it('should set payment number to number of payments plus 1', async () => {
      ...
      jest.mocked(api.getSelectedFeeRecordsDetailsWithoutExistingCompatiblePaymentGroups).mockResolvedValue({ ...aSelectedFeeRecordsDetails(), payments: [] });
    ...
    it('should set data sort values for reported fee column to order by currency alphabetically then value', async () => {
      ...
      jest.mocked(api.getSelectedFeeRecordsDetailsWithoutExistingCompatiblePaymentGroups).mockResolvedValue({
