...
const {
  amendmentUnderwriterManagerDecisionValidation,
} = require('./validation/amendmentUnderwriterManagerDecision.validate');
...
const postAmendmentAddUnderwriterManagersDecisionCoverEndDate = async (req, res) => {
  ...
  const { errorsObject, amendmentUnderwriterManagerValidationErrors } = amendmentUnderwriterManagerDecisionValidation(
    decision,
    'coverEndDate',
  );
  const { data: amendment } = await api.getAmendmentById(facilityId, amendmentId, userToken);
  ...
  try {
    ...
    if (status === 200) {
      if (amendment.changeFacilityValue) {
        return res.redirect(
          `/case/${dealId}/facility/${facilityId}/amendment/${amendmentId}/facility-value/managers-decision`,
        );
      }
