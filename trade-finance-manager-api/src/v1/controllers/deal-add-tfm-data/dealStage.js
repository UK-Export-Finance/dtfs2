 ...
 * If the status is not equal to PORTAL_DEAL_STATUS.UKEF_ACKNOWLEDGED, it returns DEAL_STAGE_TFM.APPLICATION. Otherwise,
...
const dealStage = (status, submissionType) => {
  ...
  if (status === PORTAL_DEAL_STATUS.UKEF_ACKNOWLEDGED) {
    return isSubmissionTypeAinOrMin ? DEAL_STAGE_TFM.CONFIRMED : DEAL_STAGE_TFM.APPLICATION;
  }

  console.info('Invalid deal stage with status %s and submission type %s', status, submissionType);
  return false;
};
