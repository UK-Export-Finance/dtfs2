...
describe('auth-service/index', () => {
  ...
  beforeAll(async () => {
    ...
    issueJwtAndUpdateUser.execute = jest.fn().mockResolvedValue(mockToken);
  ...
  it('should call issueJwtAndUpdateUser.execute with the retrieved TFM user', () => {
    expect(issueJwtAndUpdateUser.execute).toHaveBeenCalledTimes(1);
    expect(issueJwtAndUpdateUser.execute).toHaveBeenCalledWith(MOCK_TFM_USER);
  });
