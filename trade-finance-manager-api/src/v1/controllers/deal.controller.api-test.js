...
describe('canDealBeSubmittedToACBS()', () => {
  it('Should return `FALSE` as the deal is a MIA', async () => {
    expect(dealController.canDealBeSubmittedToACBS(MOCK_DEAL_MIA_SUBMITTED.details.submissionType)).toEqual(false);
  });
  ...
  it('Should return `TRUE` as the deal is a AIN', async () => {
    expect(dealController.canDealBeSubmittedToACBS(MOCK_DEAL_AIN_SUBMITTED.details.submissionType)).toEqual(true);
  });
});
