...
exports.queryAllFacilities = async (queryParams) => {
  ...
  for (const item of rawFacilities) {
    ...
    facility.coverEndDate = item.tfmFacilities.coverEndDate ? (new Date(item.tfmFacilities.coverEndDate)).toLocaleDateString('en-US', {
      month: 'short',
      day: 'numeric',
    }) : '';
    facility.coverEndDateEpoch = item.tfmFacilities.coverEndDate ? (new Date(item.tfmFacilities.coverEndDate)).getTime() : '';
