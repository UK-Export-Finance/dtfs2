  ...
  try {
    ...
    if (eStoreData.facilityIdentifiers.length) {
      ...
      if (facilityFoldersResponse.every((item: any) => item.status === 201)) {
        ...
        console.info('Cron job continues: eStore Facility Folder Cron Job continues to run for facilityIdentifiers: ', eStoreData.facilityIdentifiers);
        // increment the dealFolderCreationRetries by 1
        const response = await cronJobLogsCollection.findOneAndUpdate(
          { dealId: eStoreData.dealId },
          { $inc: { facilityFolderCreationRetries: 1 } },
          { returnDocument: 'after' },
        );
        // stop the Facility Folder Cron Job after 3 retries
        // this is to prevent it from running forever
        if (response?.value?.facilityFolderCreationRetries === 3) {
