...
describe('/graphql query deals', () => {
  ...
  describe('/graphql list all deals', () => {
    ...
    it('accepts requests that come from a user with role=maker', async () => {
      const { status } = await as(anHSBCMaker).post(queryBody).to('/graphql');
      expect(status).toEqual(200);
    });


    it('accepts requests that come from a user with role=checker', async () => {
      const { status } = await as(aBarclaysChecker).post(queryBody).to('/graphql');
      expect(status).toEqual(200);
    });
    ...
    it('returns a list of deals ordered by "updated", filtered by <user>.bank.id', async () => {
      const deals = [
        ...
        aDeal({ additionalRefName: 'bank1-2', bankInternalRefName: 'mockSupplyContractId', submissionType: 'mock' }),
        aDeal({ additionalRefName: 'bank2-0', bankInternalRefName: 'mockSupplyContractId', submissionType: 'mock' }),
        aDeal({ additionalRefName: 'bank2-1', bankInternalRefName: 'mockSupplyContractId', submissionType: 'mock' }),
      ];

      const bla = await as(aBarclaysMaker).post(deals[4]).to('/v1/deals');
      ...
      const { body, status } = await as(anHSBCMaker).post(queryBody).to('/graphql');
    ...
    it('returns a list of deals ordered by "updated" if <user>.bank.id == *', async () => {
      ...
      submitOrder.reverse();
      body.data.allDeals.deals.forEach((deal, index) => {
        expect(deal.bankInternalRefName).toEqual(deals[submitOrder[index]].bankInternalRefName);
      });
    });
  });
  ...
  describe('/graphql list deals pagination', () => {
    it('returns a list of deals, ordered by "updated", paginated by start/pagesize, filtered by <user>.bank.id', async () => {
      ...
      expect(body.data.allDeals.deals[1].bankInternalRefName).toEqual(deals[2].bankInternalRefName);
    ...
    it('returns a list of deals, ordered by "updated", paginated by start/pagesize, if <user>.bank.id == *', async () => {
      ...
      const { body, status } = await as(aSuperuser).post({ query: dealsPaginationQuery2 }).to('/graphql');
      expect(status).toEqual(200);

      // expect deals in reverse order - when we slice the last 3 deals we should get 2/1/0
      expect(body.data.allDeals.deals[0].bankInternalRefName).toEqual(deals[2].bankInternalRefName);
      expect(body.data.allDeals.deals[1].bankInternalRefName).toEqual(deals[1].bankInternalRefName);
      expect(body.data.allDeals.deals[2].bankInternalRefName).toEqual(deals[0].bankInternalRefName);
  ...
  describe('/graphql list deals filters', () => {
    it('returns a list of deals, ordered by "updated", filtered by equal filters', async () => {
      ...
      expect(body.data.allDeals.deals[0].bankInternalRefName).toEqual(deals[5].bankInternalRefName);
      expect(body.data.allDeals.deals[1].bankInternalRefName).toEqual(deals[3].bankInternalRefName);
      expect(body.data.allDeals.deals[2].bankInternalRefName).toEqual(deals[2].bankInternalRefName);
      expect(body.data.allDeals.deals[3].bankInternalRefName).toEqual(deals[1].bankInternalRefName);
      expect(body.data.allDeals.deals[4].bankInternalRefName).toEqual(deals[0].bankInternalRefName);
    ...
    it('returns a list of deals, ordered by "updated", filtered by not equal filter', async () => {
      ...
      // expect deals in reverse order; most recent first..
      expect(body.data.allDeals.deals[0].bankInternalRefName).toEqual(deals[7].bankInternalRefName);
      expect(body.data.allDeals.deals[1].bankInternalRefName).toEqual(deals[6].bankInternalRefName);
      expect(body.data.allDeals.deals[2].bankInternalRefName).toEqual(deals[4].bankInternalRefName);
