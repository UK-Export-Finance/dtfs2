...
describe('/v1/deals/:id/loan/change-cover-start-date', () => {
  ...
  const updateLoanCoverStartDate = async (bssDealId, bssLoanId, loan) => {
    const response = await as(testbank1Maker).put(loan).to(`/v1/deals/${bssDealId}/loan/${bssLoanId}/change-cover-start-date`);
    return response;
  };
  ...
  describe('GET /v1/deals/:id/loan/:id/change-cover-start-date', () => {
    ...
    it('401s requests that do not come from a user with role=maker', async () => {
      const { status } = await as(testbank2Maker).put({}).to(`/v1/deals/${dealId}/loan/620a1aa095a618b12da38c7b/change-cover-start-date`);
    ...
    it('401s requests if <user>.bank != <resource>/bank', async () => {
      ...
      const { status } = await as(testbank1Maker).put({}).to(`/v1/deals/${dealId}/loan/620a1aa095a618b12da38c7b/change-cover-start-date`);

      expect(status).toEqual(401);
    });

    it('404s requests for unknown deal', async () => {
      const { status } = await as(testbank1Maker).put({}).to('/v1/deals/620a1aa095a618b12da38c7b/loan/620a1aa095a618b12da38c7b/change-cover-start-date');
