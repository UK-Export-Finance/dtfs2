...
describe('/v1/deals/:id/clone', () => {
  ...
  describe('POST /v1/deals/:id/clone', () => {
    ...
    describe('with post body', () => {
      ...
      beforeEach(async () => {
        const { body: createdDeal } = await as(anTestbank2Maker).post(dealToClone).to('/v1/deals');
        const originalDealId = createdDeal._id;
        createdFacilities = await createFacilities(anTestbank2Maker, originalDealId, completedDeal.mockFacilities);

        const { body: dealAfterCreation } = await as(anTestbank2Maker).get(`/v1/deals/${originalDealId}`);
      ...
      it('clones a deal with only specific properties in `details`. Wipes `previousStatus`, `comments`, `editedBy` `ukefComments`, `ukefDecision`, changes `maker` to the user making the request, marks status `Draft`', async () => {
        ...
        const { body } = await as(aTestbank1Maker).post(clonePostBody).to(`/v1/deals/${originalDeal._id}/clone`);

        const { body: cloned } = await as(aTestbank1Maker).get(`/v1/deals/${body._id}`);
        ...
        expect(cloned.deal.maker.username).toEqual(aTestbank1Maker.username);
        expect(cloned.deal.maker.roles).toEqual(aTestbank1Maker.roles);
        expect(cloned.deal.maker.bank).toEqual(aTestbank1Maker.bank);
        expect(cloned.deal.maker.firstname).toEqual(aTestbank1Maker.firstname);
        expect(cloned.deal.maker.surname).toEqual(aTestbank1Maker.surname);
        expect(cloned.deal.maker._id).toEqual(aTestbank1Maker._id);

        expect(cloned.deal.bank).toEqual(aTestbank1Maker.bank);
      ...
      it('updates the audit details to the user cloning the deal', async () => {
        ...
        expect(originalDeal.auditRecord).toEqual(generateParsedMockPortalUserAuditDatabaseRecord(anTestbank2Maker._id));
        expect(cloned.deal.auditRecord).toEqual(generateParsedMockPortalUserAuditDatabaseRecord(aTestbank1Maker._id));
      });
      ...
      it('should clone eligibility agent details', async () => {
        ...
        const createDeal = await as(anTestbank2Maker).post(dealToCloneWithEligibilityAgentDetails).to('/v1/deals');
        const { _id: dealId } = createDeal.body;

        const { body } = await as(aTestbank1Maker).post(clonePostBody).to(`/v1/deals/${dealId}/clone`);
      ...
      describe('when deal submissionType is MIN', () => {
        it('should change deal submissionType to MIA', async () => {
          ...
          const { body: minDealBody } = await as(anTestbank2Maker).put(minDeal).to(`/v1/deals/${minDeal._id}`);

          const { body } = await as(aTestbank1Maker).post(clonePostBody).to(`/v1/deals/${minDealBody._id}/clone`);
      ...
      it('clones a deal with only specific bondTransactions fields', async () => {
        ...
        const clonedDealResponse = await as(anTestbank2Maker).post(clonePostBody).to(`/v1/deals/${originalDeal._id}/clone`);
        const clonedDealId = clonedDealResponse.body._id;

        const getDealResponse = await as(anTestbank2Maker).get(`/v1/deals/${clonedDealId}`);
      ...
      it('returns empty facilities, bondTransactions and loanTransactions when empty in original deal', async () => {
        ...
        const { body } = await as(anTestbank2Maker).post(dealWithEmptyBondsAndLoans).to('/v1/deals');
        originalDeal = body;
        ...
        const { body: responseBody } = await as(anTestbank2Maker).post(clonePostBody).to(`/v1/deals/${originalDeal._id}/clone`);

        const { body: cloned } = await as(anTestbank2Maker).get(`/v1/deals/${responseBody._id}`);
      ...
      describe('when required fields are missing', () => {
        it('returns validation errors', async () => {
          ...
          const { body } = await as(anTestbank2Maker).post(clonePostBody).to(`/v1/deals/${originalDeal._id}/clone`);
