...
describe('/v1/deals/:id/eligibility-documentation', () => {
  ...
  describe('GET /v1/deals/:id/eligibility-documentation/:fieldname/:filename', () => {
    ...
    let dealId;
    let aTestbank1EligibilityDocumentationFileUrl;
    ...
    beforeEach(async () => {
      ...
      await as(aTestbank1Maker).putMultipartForm({}, files).to(`/v1/deals/${dealId}/eligibility-documentation`);

      aTestbank1EligibilityDocumentationFileUrl = `/v1/deals/${dealId}/eligibility-documentation/${fieldname}/${filename.replaceAll(/-/g, '_')}`;
    });

    withClientAuthenticationTests({
      makeRequestWithoutAuthHeader: () => get(aTestbank1EligibilityDocumentationFileUrl),
      makeRequestWithAuthHeader: (authHeader) => get(aTestbank1EligibilityDocumentationFileUrl, { headers: { Authorization: authHeader } }),
    });

    withRoleAuthorisationTests({
      allowedRoles: [MAKER, CHECKER, READ_ONLY, ADMIN],
      getUserWithRole: (role) => testUsers().withRole(role).withBankName('Bank 1').one(),
      makeRequestAsUser: (user) => as(user).get(aTestbank1EligibilityDocumentationFileUrl),
      successStatusCode: 200,
    });

    it('downloads an uploaded file', async () => {
      const { status, text, header } = await as(aTestbank1Maker).get(aTestbank1EligibilityDocumentationFileUrl);
    ...
    it('does not allow download of file from a different user organisation', async () => {
      const { status, text, header } = await as(anTestbank2Maker).get(aTestbank1EligibilityDocumentationFileUrl);
    ...
    it("returns 404 if deal doesn't exist", async () => {
      const { status, text, header } = await as(aTestbank1Maker).get(`/v1/deals/620a1aa095a618b12da38c7b/eligibility-documentation/${fieldname}/${filename}`);
    ...
    it('returns 404 if no deal files have been uploaded', async () => {
      ...
      const { status, text, header } = await as(aTestbank1Maker).get(`/v1/deals/${newId}/eligibility-documentation/${fieldname}/${filename}`);
    ...
    it("returns 404 is requested file doesn't exist", async () => {
      const { status, text, header } = await as(aTestbank1Maker).get(`/v1/deals/${dealId}/eligibility-documentation/${fieldname}/non-exisitant-file.txt`);
  ...
  describe('PUT /v1/deals/:id/eligibility-documentation', () => {
    ...
    it("401s requests if user tries to update deal it doesn't have permission for", async () => {
      ...
      const { status } = await as(anTestbank2Maker).putMultipartForm({}, files).to(`/v1/deals/${newId}/eligibility-documentation`);
    ...
    it('updates eligibility status depending on whether validation errors occur', async () => {
      ...
      const updatedDeal = await as(aTestbank1Maker).put(updatedECCompleted).to(`/v1/deals/${newId}/eligibility-criteria`);
      ...
      const updatedValidDeal = await as(aTestbank1Maker).putMultipartForm({}, files).to(`/v1/deals/${newId}/eligibility-documentation`);
      ...
      const updatedInvalidDeal = await as(aTestbank1Maker).putMultipartForm(deleteFileData, []).to(`/v1/deals/${newId}/eligibility-documentation`);
    ...
    it('uploads a file with the correct type', async () => {
      ...
      const { status, body } = await as(aTestbank1Maker).putMultipartForm({}, files).to(`/v1/deals/${newId}/eligibility-documentation`);
    ...
    it('does not create duplicate entry if same file is reuploaded', async () => {
      ...
      await as(aTestbank1Maker).putMultipartForm({}, files).to(`/v1/deals/${newId}/eligibility-documentation`);
    ...
    it('deletes an uploaded file', async () => {
      ...
      const uploadedDealRes = await as(aTestbank1Maker).putMultipartForm({}, files).to(`/v1/deals/${newId}/eligibility-documentation`);
      ...
      const { status, body } = await as(aTestbank1Maker).putMultipartForm(deleteFileData, []).to(`/v1/deals/${newId}/eligibility-documentation`);
    ...
    it('saves security text field', async () => {
      ...
      const { status, body } = await as(aTestbank1Maker).putMultipartForm(textFields).to(`/v1/deals/${newId}/eligibility-documentation`);
