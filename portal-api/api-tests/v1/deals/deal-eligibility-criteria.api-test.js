...
describe('/v1/deals/:id/eligibility-criteria', () => {
  ...
  describe('PUT /v1/deals/:id/eligibility-criteria', () => {
    ...
    it('401s requests that do not come from a user with role=maker', async () => {
      const { status } = await as(testUser).put(updatedECPartial).to('/v1/deals/620a1aa095a618b12da38c7b/eligibility-criteria');
    ...
    it('401s requests if <user>.bank != <resource>/bank', async () => {
      ...
      const { status } = await as(anTestbank2Maker).put(updatedECPartial).to(`/v1/deals/${newId}/eligibility-criteria`);

      expect(status).toEqual(401);
    });

    it('404s requests for unknown resources', async () => {
      const { status } = await as(aTestbank1Maker).put(updatedECPartial).to('/v1/deals/620a1aa095a618b12da38c7b/eligibility-criteria');
    ...
    it('updates the eligibility criteria with lastUpdated timestamp on each update', async () => {
      ...
      // first update
      await as(aTestbank1Maker).put(updatedECPartial).to(`/v1/deals/${newId}/eligibility-criteria`);

      const { status, body } = await as(aTestbank1Maker).get(`/v1/deals/${newId}`);
      ...
      const { body: secondUpdateBody } = await as(aTestbank1Maker).get(`/v1/deals/${newId}`);
    ...
    it('updates all the eligibility criteria without validation error', async () => {
      ...
      const { status, body } = await as(aTestbank1Maker).put(updatedECCompleted).to(`/v1/deals/${newId}/eligibility-criteria`);
    ...
    it('updates some of the eligibility criteria and generates validation errors', async () => {
      ...
      const { status, body } = await as(aTestbank1Maker).put(updatedECPartial).to(`/v1/deals/${newId}/eligibility-criteria`);
    ...
    it('generated validation errors if criteria11 is false but extra info not entered', async () => {
      ...
      const { status, body } = await as(aTestbank1Maker).put(updatedECPostcode).to(`/v1/deals/${newId}/eligibility-criteria`);
    ...
    it('generated postcode validation error if criteria11 is false and country = GBR but postcode not entered', async () => {
      ...
      const { status, body } = await as(aTestbank1Maker).put(updatedECCountry).to(`/v1/deals/${newId}/eligibility-criteria`);
    ...
    it('should generate address country validation error if the the country is `undefined`', async () => {
      ...
      const { status, body } = await as(aTestbank1Maker).put(ec11UndefinedCountry).to(`/v1/deals/${newId}/eligibility-criteria`);
    ...
    it('updates criteria 11 extra info in criteria11 is false', async () => {
      ...
      const { status, body } = await as(aTestbank1Maker).put(updatedECCriteria11WithExtraInfo).to(`/v1/deals/${newId}/eligibility-criteria`);
    ...
    it('limits length of agent name to 150 characters', async () => {
      ...
      const { status, body } = await as(aTestbank1Maker).put(updatedECCriteria11WithExtraInfoLongAgent).to(`/v1/deals/${newId}/eligibility-criteria`);
    ...
    it('does not generate town mandatory error if criteria11 is false and country = GBR and town not entered', async () => {
      ...
      const { status, body } = await as(aTestbank1Maker).put(updatedECCriteria11WithExtraInfoNotGBR).to(`/v1/deals/${newId}/eligibility-criteria`);
    ...
    it('removes criteria 11 extra info when criteria11 is changed from false to true', async () => {
      ...
      const { body } = await as(aTestbank1Maker).put(updatedECCriteria11WithExtraInfo).to(`/v1/deals/${newId}/eligibility-criteria`);
      expect(body.eligibility).toMatchObject(criteria11ExtraInfo);
      ...
      const { status, body: body2 } = await as(aTestbank1Maker).put(updateCriteria11Eligibility).to(`/v1/deals/${newId}/eligibility-criteria`);
    ...
    it('updates the submissionType to AIN if all true answers', async () => {
      ...
      const { status, body } = await as(aTestbank1Maker).put(updatedECCompletedAllTrue).to(`/v1/deals/${newId}/eligibility-criteria`);
    ...
    it('updates the lastUpdated timestamp on each update', async () => {
      ...
      const { body: firstUpdate } = await as(aTestbank1Maker).put(updatedECCompleted).to(`/v1/deals/${dealId}/eligibility-criteria`);

      const firstUpdateTimeStamp = firstUpdate.eligibility.lastUpdated;

      const { body: secondUpdate } = await as(aTestbank1Maker).put(updatedECCompleted).to(`/v1/deals/${dealId}/eligibility-criteria`);
