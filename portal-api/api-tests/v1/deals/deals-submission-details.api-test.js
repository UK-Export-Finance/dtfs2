...
describe('/v1/deals/:id/submission-details', () => {
  ...
  describe('GET /v1/deals/:id/submission-details', () => {
    ...
    it('401s requests if <user>.bank != <resource>/bank', async () => {
      const { status } = await as(anTestbank2Maker).get(oneDealSubmissionDetailsUrl);

      expect(status).toEqual(401);
    });

    it('404s requests for unknown ids', async () => {
      const { status } = await as(aTestbank1Maker).get('/v1/deals/620a1aa095a618b12da38c7b/submission-details');
    ...
    it('returns the requested resource', async () => {
      const { status, body } = await as(aTestbank1Maker).get(oneDealSubmissionDetailsUrl);
  ...
  describe('PUT /v1/deals/:id/submission-details', () => {
    ...
    it('401s requests if <user>.bank != <resource>/bank', async () => {
      ...
      const { status } = await as(aTestbank1Maker).put(statusUpdate).to(`/v1/deals/${body._id}/submission-details`);
    ...
    it('returns the updated submission-details with country fields as objects', async () => {
      ...
      const { status, body } = await as(anTestbank2Maker).put(submissionDetails).to(`/v1/deals/${createdDeal._id}/submission-details`);
    ...
    describe('when a country field changes', () => {
      it('returns the updated submission-details with country objects', async () => {
        ...
        const { status } = await as(anTestbank2Maker).put(submissionDetails).to(`/v1/deals/${createdDeal._id}/submission-details`);
        expect(status).toEqual(200);
        ...
        const updatedSubmissionDetails = await as(anTestbank2Maker).put(body).to(`/v1/deals/${createdDeal._id}/submission-details`);
    ...
    describe('when supplyContractCurrency changes', () => {
      it('returns the updated submission-details with supplyContractCurrency as a currency object', async () => {
        ...
        const { body, status } = await as(anTestbank2Maker).put(submissionDetails).to(`/v1/deals/${createdDeal._id}/submission-details`);
        expect(status).toEqual(200);
        ...
        const updatedSubmissionDetails = await as(anTestbank2Maker).put(updateBody).to(`/v1/deals/${createdDeal._id}/submission-details`);
        expect(updatedSubmissionDetails.status).toEqual(200);
    ...
    it('updates the deal', async () => {
      ...
      await as(anTestbank2Maker).put(submissionDetails).to(`/v1/deals/${createdDeal._id}/submission-details`);
    ...
    it('updates the deals updatedAt field', async () => {
      ...
      const { body: dealInOriginalShape } = await as(anTestbank2Maker).get(`/v1/deals/${createdDeal._id}`);
    ...
    it('updates deal.exporter.companyName with the provided `supplier-name` field', async () => {
      ...
      await as(anTestbank2Maker).get(`/v1/deals/${createdDeal._id}`);
