  ...
  describe('api calls', () => {
    ...
    describe('/v1/users/reset-password/:token', () => {
      it('should return error for empty current password field', async () => {
        const { body } = await as()
          .post({ currentPassword: '', password: '1', passwordConfirm: '1' })
          .to('/v1/users/reset-password/mock123');
      ...
      it('should return error for empty new password field', async () => {
        const { body } = await as()
          .post({ currentPassword: '1', password: '', passwordConfirm: '1' })
          .to('/v1/users/reset-password/mock123');
        expect(body.success).toEqual(false);
      ...
      it('should return error for empty new confirm password field', async () => {
        const { body } = await as()
          .post({ currentPassword: '1', password: '1', passwordConfirm: '' })
          .to('/v1/users/reset-password/mock123');
        expect(body.success).toEqual(false);
      ...
      it('should return error for new passwords do not match', async () => {
        const { body } = await as()
          .post({ currentPassword: '123', password: '1', passwordConfirm: '2' })
          .to('/v1/users/reset-password/mock123');
        expect(body.success).toEqual(false);
