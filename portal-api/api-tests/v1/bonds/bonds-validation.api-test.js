...
describe('/v1/deals/:id/bond', () => {
  ...
  const updateBondInDeal = async (theDealId, bond) => {
    const response = await as(aTestbank1Maker).put(bond).to(`/v1/deals/${theDealId}/bond/${bondId}`);
  ...
  beforeEach(async () => {
    ...
    const bondResponse = await as(aTestbank1Maker).put({}).to(`/v1/deals/${dealId}/bond/create`);
  ...
  describe('GET /v1/deals/:id/bond/:id', () => {
    it('returns a bond with validationErrors for all required fields', async () => {
      const { body } = await as(aTestbank1Maker).get(`/v1/deals/${dealId}/bond/${bondId}`);
  ...
  describe('PUT /v1/deals/:id/bond/:bondId', () => {
    ...
    it('returns 400 with validation errors', async () => {
      const { body, status } = await as(aTestbank1Maker).put({}).to(`/v1/deals/${dealId}/bond/${bondId}`);
