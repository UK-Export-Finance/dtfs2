...
describe('/v1/deals/:id/bond/:bondId/issue-facility', () => {
  ...
  const updateDeal = async (dealId, body) => {
    const result = await as(testbank1Maker).put(body).to(`/v1/deals/${dealId}`);
    return result.body;
  };

  const updateBond = async (dealId, bondId, body) => {
    const result = await as(testbank1Maker).put(body).to(`/v1/deals/${dealId}/bond/${bondId}`);
    return result.body;
  };

  const updateBondIssuance = async (dealId, bondId, bond) => {
    const response = await as(testbank1Maker).put(bond).to(`/v1/deals/${dealId}/bond/${bondId}/issue-facility`);
  ...
  describe('PUT /v1/deals/:id/bond/:bondId/issue-facility', () => {
    it('returns 400 with validation errors', async () => {
      ...
      const { body, status } = await as(testbank1Maker).put({}).to(`/v1/deals/${dealID}/bond/${bondID}/issue-facility`);
