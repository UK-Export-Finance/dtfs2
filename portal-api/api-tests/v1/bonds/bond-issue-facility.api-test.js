...
describe('/v1/deals/:id/bond/:id/issue-facility', () => {
  ...
  const createBond = async () => {
    ...
    const { status, body } = await as(aTestbank1Maker).put(allBondFields).to(`/v1/deals/${dealId}/bond/${bondId}`);
    expect(body.hasBeenIssued).toEqual(false);
    expect(status).toEqual(200);
  };

  const putIssueFacility = async (issueDealId, issueBondId, body) => {
    const response = await as(aTestbank1Maker).put(body).to(`/v1/deals/${issueDealId}/bond/${issueBondId}/issue-facility`);
    return response;
  };
  ...
  describe('PUT /v1/deals/:id/bond/:id/issue-facility', () => {
    it('should return 401 when user cannot access the deal', async () => {
      const { status } = await as(anTestbank2Maker).put({}).to(`/v1/deals/${dealId}/bond/${bondId}/issue-facility`);
    ...
    it('should remove bond status when issueFacilityDetailsStarted already exists in the bond', async () => {
      ...
      await as(aTestbank1Maker).put(bondWithIssueFacilityDetailsStarted).to(`/v1/deals/${dealId}/bond/${bondId}`);
