...
openRouter
    ...
    .post(passport.authenticate(partial2faTokenPassportStrategy, { session: false }), param('userId').isMongoId().withMessage('Value must be a valid MongoId'), param('signInToken')
    .isHexadecimal()
    ...
    .withMessage(`Value must be ${SIGN_IN_LINK.TOKEN_HEX_LENGTH} characters long`), handleExpressValidatorResult, users.loginWithSignInLink);
...
authRouter.route('/deals/:id/eligibility-documentation').put(validateUserHasAtLeastOneAllowedRole({ allowedRoles: [MAKER] }), (req, res, next) => {
...
openRouter.get('/validate-partial-2fa-token', passport.authenticate(partial2faTokenPassportStrategy, { session: false }), (_req, res) => res.status(200).send());
// bank-validator
authRouter.get('/validate/bank', (req, res) => banks.validateBank(req, res));
// utilisation report service
authRouter.route('/utilisation-reports').post(validateUserHasAtLeastOneAllowedRole({ allowedRoles: [PAYMENT_REPORT_OFFICER] }), (req, res, next) => {
...
authRouter
    ...
    .get(validateUserHasAtLeastOneAllowedRole({ allowedRoles: [PAYMENT_REPORT_OFFICER] }), bankIdValidation, handleExpressValidatorResult, validateUserAndBankIdMatch, utilisationReportControllers.getPreviousReportsByBankId);
authRouter
    .route('/banks/:bankId/utilisation-reports/last-uploaded')
    .get(validateUserHasAtLeastOneAllowedRole({ allowedRoles: [PAYMENT_REPORT_OFFICER] }), bankIdValidation, handleExpressValidatorResult, validateUserAndBankIdMatch, utilisationReportControllers.getLastUploadedReportByBankId);
authRouter
    .route('/banks/:bankId/due-report-periods')
    .get(validateUserHasAtLeastOneAllowedRole({ allowedRoles: [PAYMENT_REPORT_OFFICER] }), bankIdValidation, handleExpressValidatorResult, validateUserAndBankIdMatch, utilisationReportControllers.getDueReportPeriodsByBankId);
authRouter
    .route('/banks/:bankId/utilisation-report-download/:_id')
    .get(validateUserHasAtLeastOneAllowedRole({ allowedRoles: [PAYMENT_REPORT_OFFICER] }), bankIdValidation, mongoIdValidation, handleExpressValidatorResult, validateUserAndBankIdMatch, utilisationReportControllers.getReportDownload);
