...
const cloneFacilities = (currentDealId, newDealId) => tslib_1.__awaiter(void 0, void 0, void 0, function* () {
    ...
    const collection = yield db.getCollection(facilitiesCollection);
    // get all existing facilities
    const allFacilities = yield collection
        .aggregate([
        {
    ...
    if (allFacilities.length) {
        ...
        yield collection.insertMany(allFacilities);
    }
});
const cloneDeal = (dealId, bankInternalRefName, additionalRefName, maker, userId, bank) => tslib_1.__awaiter(void 0, void 0, void 0, function* () {
    ...
    const collection = yield db.getCollection(applicationCollection);
    ...
    const existingDeal = yield collection.findOne({ _id: { $eq: ObjectId(dealId) }, 'bank.id': { $eq: bank.id } });
    if (existingDeal) {
        ...
        const createdApplication = yield collection.insertOne(clonedDeal);
...
exports.clone = (req, res) => tslib_1.__awaiter(void 0, void 0, void 0, function* () {
    const { body: { dealId: existingDealId, bankInternalRefName, additionalRefName, userId, bank }, } = req;
    const validateErrs = validateApplicationReferences(req.body);
    if (validateErrs) {
        return res.status(422).send(validateErrs);
    }
    // clone GEF deal
    const response = yield cloneDeal(existingDealId, bankInternalRefName, additionalRefName, req.user, userId, bank);
    if (response.status === 200) {
        const { newDealId } = response;
        // clone the corresponding facilities
        yield cloneFacilities(existingDealId, newDealId);
