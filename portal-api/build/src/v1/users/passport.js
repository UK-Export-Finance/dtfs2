...
const baseAuthenticationConfiguration = ({ name, passport, additionalPayloadValidation, additionalUserValidation = null, getAdditionalReturnedFields = () => { }, }) => {
    passport.use(name, new JwtStrategy(options, (jwtPayload, done) => tslib_1.__awaiter(void 0, void 0, void 0, function* () {
        findByUsername(jwtPayload.username, (error, user) => {
            ...
            return done(null, Object.assign(Object.assign({}, sanitize(user)), additionalFields));
        });
    })));
};
const loginCompleteAuth = (passport, userService) => {
    const name = 'login-complete';
    const additionalPayloadValidation = (jwtPayload) => (jwtPayload.loginStatus === LOGIN_STATUSES.VALID_2FA ? PASSPORT_VALIDATION_RESULTS.PASSED : PASSPORT_VALIDATION_RESULTS.FAILED);
...
const loginInProgressAuth = (passport) => {
    const additionalPayloadValidation = (jwtPayload) => (jwtPayload.loginStatus === LOGIN_STATUSES.VALID_USERNAME_AND_PASSWORD ? PASSPORT_VALIDATION_RESULTS.PASSED : PASSPORT_VALIDATION_RESULTS.FAILED);
