 ...
 */
module.exports.login = (username, password, userService) => new Promise((resolve) => {
    findByUsername(username, (error, user) => tslib_1.__awaiter(void 0, void 0, void 0, function* () {
        if (error) {
        ...
        if (passwordIncorrect) {
            yield incrementFailedLoginCount(user);
            return resolve({ error: usernameOrPasswordIncorrect });
        }
        try {
        ...
        const _a = utils.issueValidUsernameAndPasswordJWT(user), { sessionIdentifier } = _a, tokenObject = tslib_1.__rest(_a, ["sessionIdentifier"]);
