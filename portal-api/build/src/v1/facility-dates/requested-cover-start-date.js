...
const hasAllRequestedCoverStartDateValues = (facility) => {
    const { 'requestedCoverStartDate-day': requestedCoverStartDateDay, 'requestedCoverStartDate-month': requestedCoverStartDateMonth, 'requestedCoverStartDate-year': requestedCoverStartDateYear, } = facility;
...
exports.updateRequestedCoverStartDate = (facility) => {
    ...
    if (hasAllRequestedCoverStartDateValues(facility)) {
        const { 'requestedCoverStartDate-day': requestedCoverStartDateDay, 'requestedCoverStartDate-month': requestedCoverStartDateMonth, 'requestedCoverStartDate-year': requestedCoverStartDateYear, } = facility;
        modifiedFacility.requestedCoverStartDate = getDateAsEpochMillisecondString(getStartOfDateFromDayMonthYearStringsReplicatingMoment(requestedCoverStartDateDay, requestedCoverStartDateMonth, requestedCoverStartDateYear));
