 ...
 */
const getReportDueDate = () => tslib_1.__awaiter(void 0, void 0, void 0, function* () {
    const bankHolidays = yield externalApi.bankHolidays.getBankHolidayDatesForRegion(BANK_HOLIDAY_REGION.ENGLAND_AND_WALES);
 ...
 */
const getFormattedReportDueDate = () => tslib_1.__awaiter(void 0, void 0, void 0, function* () {
 ...
 * @returns {Promise<Date>}
 */
const getReportOverdueChaserDate = () => tslib_1.__awaiter(void 0, void 0, void 0, function* () {
    const bankHolidays = yield externalApi.bankHolidays.getBankHolidayDatesForRegion(BANK_HOLIDAY_REGION.ENGLAND_AND_WALES);
 ...
 */
const getIsReportSubmitted = (bank) => tslib_1.__awaiter(void 0, void 0, void 0, function* () {
    const reportPeriod = getReportPeriod();
    const uploadedReportsInReportPeriod = yield api.getUtilisationReports(bank.id, {
        reportPeriod,
...
const getEmailRecipient = (paymentOfficerTeam, bankName) => {
    if (!(paymentOfficerTeam === null || paymentOfficerTeam === void 0 ? void 0 : paymentOfficerTeam.teamName)) {
 ...
 */
const sendEmailForBank = (_a) => tslib_1.__awaiter(void 0, [_a], void 0, function* ({ emailDescription, sendEmailCallback, bank }) {
    const { name: bankName, paymentOfficerTeam } = bank;
    const paymentOfficerTeamEmail = paymentOfficerTeam === null || paymentOfficerTeam === void 0 ? void 0 : paymentOfficerTeam.email;
    try {
 ...
 * @returns {Promise<void>}
 */
const sendEmailToAllBanksWhereReportNotReceived = (_b) => tslib_1.__awaiter(void 0, [_b], void 0, function* ({ emailDescription, sendEmailCallback }) {
    console.info(`Attempting to send ${emailDescription} emails`);
    const banks = yield api.getAllBanks();
    const reportPeriod = getFormattedReportPeriod();
    for (const bank of banks) {
        const { name: bankName, id: bankId } = bank;
        try {
            const isReportSubmitted = yield getIsReportSubmitted(bank);
            if (isReportSubmitted) {
                console.info(`Not sending '${emailDescription}' email to '${bankName}' (bank ID: ${bankId}) - report has already been submitted for ${reportPeriod} report period`);
            }
            else {
                yield sendEmailForBank({ emailDescription, sendEmailCallback, bank });
