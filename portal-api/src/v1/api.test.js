...
import {
  ...
  ALL_AMENDMENT_SUBMITTED_STATUSES,
...
describe('getFacilityAmendmentsOnDeal', () => {
  ...
  const params = { statuses: ALL_AMENDMENT_SUBMITTED_STATUSES, types };
  const mockResponseData = [
    { dealId, facilityId, amendmentId: 'amendment1', types: [AMENDMENT_TYPES.PORTAL], status: PORTAL_AMENDMENT_STATUS.ACKNOWLEDGED },
    { dealId, facilityId, amendmentId: 'amendment2', types: [AMENDMENT_TYPES.PORTAL], status: PORTAL_AMENDMENT_STATUS.FURTHER_MAKERS_INPUT_REQUIRED },
  ...
  it('should return all submitted amendments when the API call is successful', async () => {
    ...
    // Act
    const response = await getFacilityAmendmentsOnDeal(dealId, ALL_AMENDMENT_SUBMITTED_STATUSES, types);

    // Assert
    expect(response).toMatchObject(expected.data);
    expect(axios).toHaveBeenCalledTimes(1);
    expect(axios).toHaveBeenCalledWith({ method: 'get', url: `${DTFS_CENTRAL_API_URL}/v1/portal/deals/${dealId}/amendments`, params, headers });
  });
  ...
  it('should throw an error when the API call fails', async () => {
    ...
    // Act
    const response = getFacilityAmendmentsOnDeal(dealId, ALL_AMENDMENT_SUBMITTED_STATUSES, types);
  ...
  it('should return all amendments when statuses are an empty array', async () => {
    ...
    // Act
    const response = await getFacilityAmendmentsOnDeal(dealId, emptyStatuses, types);
