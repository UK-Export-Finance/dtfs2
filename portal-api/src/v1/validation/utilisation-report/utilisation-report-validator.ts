...
export const validateReportIsInReportNotReceivedState = (report: UtilisationReportResponseBody): void => {
  if (report.status !== UTILISATION_REPORT_STATUS.REPORT_NOT_RECEIVED) {
    throw new InvalidReportStatusError(
      `Expected report to be in '${UTILISATION_REPORT_STATUS.REPORT_NOT_RECEIVED}' state (was actually in '${report.status}' state)`,
    );
  }
};
