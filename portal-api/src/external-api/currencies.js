...
const getCurrencies = async () => {
  const response = await axios({
    ...
    console.error('Error retrieving currencies from External API. %o %o', error?.response?.data, error?.status);
...
const getCurrency = async (id) => {
  ...
  const response = await axios({
    ...
    console.error('Error retrieving currency from External API. %o %o', error?.response?.data, error?.status);
    return {
      status: 404,
