...
df.app.orchestration('acbs-amend-facility', function* amendFacility(context) {
  ...
  try {
    ...
    const isAmendmentContainingCovenantAmendment = Boolean(amount || amendment.facilityGuaranteeDates.guaranteeExpiryDate);
    if (isAmendmentContainingCovenantAmendment) {
      facilityCovenantRecord = context.df.callSubOrchestrator('acbs-amend-facility-covenant-record', {
        facilityId,
        amendments,
      });
      ...
      facilityCovenantRecord = {
        result: `Facility covenant will not be amended.`,
      };
    }

    // 5. SOF: Facility Fee Record (FCR)
    const isAmendmentContainingFeeAmendment = Boolean(amount || amendment.facilityGuaranteeDates.guaranteeExpiryDate);
    if (isAmendmentContainingFeeAmendment) {
      facilityFeeRecord = context.df.callSubOrchestrator('acbs-amend-facility-fee-record', {
        facilityId,
        amendments,
      });
      ...
      facilityFeeRecord = {
        result: `Facility fee will not be amended.`,
      };
    }
    return {
      facilityId,
      ...
      facilityFeeRecord: facilityFeeRecord.result,
    };
  } catch (error) {
