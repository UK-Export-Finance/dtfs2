import { CurrencyInterface } from '../interfaces';

export const CURRENCY = {
  AED: 'AED',
  AUD: 'AUD',
  BRL: 'BRL',
  BWP: 'BWP',
  CAD: 'CAD',
  CHF: 'CHF',
  CZK: 'CZK',
  DKK: 'DKK',
  DZD: 'DZD',
  EGP: 'EGP',
  EUR: 'EUR',
  GBP: 'GBP',
  HKD: 'HKD',
  ILS: 'ILS',
  INR: 'INR',
  IQD: 'IQD',
  JOD: 'JOD',
  JPY: 'JPY',
  KWD: 'KWD',
  LKR: 'LKR',
  LSL: 'LSL',
  MWK: 'MWK',
  MXN: 'MXN',
  MYR: 'MYR',
  NGN: 'NGN',
  NOK: 'NOK',
  NZD: 'NZD',
  OMR: 'OMR',
  PKR: 'PKR',
  PLN: 'PLN',
  QAR: 'QAR',
  SAR: 'SAR',
  SEK: 'SEK',
  SGD: 'SGD',
  THB: 'THB',
  TWD: 'TWD',
  USD: 'USD',
  XAF: 'XAF',
  XAU: 'XAU',
  ZAR: 'ZAR',
  BHD: 'BHD',
  CNY: 'CNY',
  PHP: 'PHP',
  UGX: 'UGX',
  KES: 'KES',
  MOP: 'MOP',
  CLP: 'CLP',
  HUF: 'HUF',
  ISK: 'ISK',
  IDR: 'IDR',
  MUR: 'MUR',
  PEN: 'PEN',
  RUB: 'RUB',
  KRW: 'KRW',
  TRY: 'TRY',
  UYU: 'UYU',
  ZMK: 'ZMK',
  MAD: 'MAD',
} as const;

/**
 * A list of currency objects, each containing details about a specific currency.
 *
 * Each currency object includes:
 * - `currencyId`: A unique identifier for the currency.
 * - `text`: A string representing the currency code and its full name.
 * - `id`: The currency code.
 * - `disabled` (optional): A boolean indicating if the currency is disabled.
 *
 * Example:
 * ```typescript
 * {
 *   currencyId: 1,
 *   text: 'AED - U.A.E. Dirham',
 *   id: 'AED',
 * }
 * ```
 */
export const CURRENCIES: CurrencyInterface[] = [
  {
    currencyId: 1,
    text: 'AED - U.A.E. Dirham',
    id: 'AED',
  },
  {
    currencyId: 2,
    text: 'AUD - Australian Dollars',
    id: 'AUD',
  },
  {
    currencyId: 3,
    text: 'BRL - Brazilian Real',
    id: 'BRL',
  },
  {
    currencyId: 4,
    text: 'BWP - Botswana Pula',
    id: 'BWP',
  },
  {
    currencyId: 5,
    text: 'CAD - Canadian Dollars',
    id: 'CAD',
  },
  {
    currencyId: 6,
    text: 'CHF - Swiss Francs',
    id: 'CHF',
  },
  {
    currencyId: 7,
    text: 'CZK - Czech Koruna',
    id: 'CZK',
  },
  {
    currencyId: 8,
    text: 'DKK - Danish Krone',
    id: 'DKK',
  },
  {
    currencyId: 9,
    text: 'DZD - Algerian Dinar',
    id: 'DZD',
    disabled: true,
  },
  {
    currencyId: 10,
    text: 'EGP - Egyptian Pounds',
    id: 'EGP',
  },
  {
    currencyId: 11,
    text: 'EUR - Euros',
    id: 'EUR',
  },
  {
    currencyId: 12,
    text: 'GBP - UK Sterling',
    id: 'GBP',
  },
  {
    currencyId: 13,
    text: 'HKD - Hong Kong Dollars',
    id: 'HKD',
  },
  {
    currencyId: 14,
    text: 'ILS - Israeli Shekels',
    id: 'ILS',
  },
  {
    currencyId: 15,
    text: 'INR - Indian Rupees',
    id: 'INR',
  },
  {
    currencyId: 16,
    text: 'IQD - Iraqi Dinar',
    id: 'IQD',
    disabled: true,
  },
  {
    currencyId: 17,
    text: 'JOD - Jordanian Dinar',
    id: 'JOD',
  },
  {
    currencyId: 18,
    text: 'JPY - Japanese Yen',
    id: 'JPY',
  },
  {
    currencyId: 19,
    text: 'KWD - Kuwaiti Dinar',
    id: 'KWD',
  },
  {
    currencyId: 20,
    text: 'LKR - Sri Lankan Rupees',
    id: 'LKR',
  },
  {
    currencyId: 21,
    text: 'LSL - Lesotho Maluti',
    id: 'LSL',
    disabled: true,
  },
  {
    currencyId: 22,
    text: 'MWK - Malawi Kwacha',
    id: 'MWK',
    disabled: true,
  },
  {
    currencyId: 23,
    text: 'MXN - Mexican Pesos',
    id: 'MXN',
  },
  {
    currencyId: 24,
    text: 'MYR - Malaysian Ringgitts',
    id: 'MYR',
  },
  {
    currencyId: 25,
    text: 'NGN - Nigerian Naira',
    id: 'NGN',
  },
  {
    currencyId: 26,
    text: 'NOK - Norwegian Krone',
    id: 'NOK',
  },
  {
    currencyId: 27,
    text: 'NZD - New Zealand Dollars',
    id: 'NZD',
  },
  {
    currencyId: 28,
    text: 'OMR - Omani Rials',
    id: 'OMR',
  },
  {
    currencyId: 29,
    text: 'PKR - Pakistan Rupees',
    id: 'PKR',
  },
  {
    currencyId: 30,
    text: 'PLN - Polish Zloty',
    id: 'PLN',
  },
  {
    currencyId: 31,
    text: 'QAR - Qatar Rials',
    id: 'QAR',
  },
  {
    currencyId: 32,
    text: 'SAR - Saudi Arabian Riyals',
    id: 'SAR',
  },
  {
    currencyId: 33,
    text: 'SEK - Swedish Krona',
    id: 'SEK',
  },
  {
    currencyId: 34,
    text: 'SGD - Singapore Dollars',
    id: 'SGD',
  },
  {
    currencyId: 35,
    text: 'THB - Thai Bahts',
    id: 'THB',
  },
  {
    currencyId: 36,
    text: 'TWD - Taiwan Dollars',
    id: 'TWD',
  },
  {
    currencyId: 37,
    text: 'USD - US Dollars',
    id: 'USD',
  },
  {
    currencyId: 38,
    text: 'XAF - CFA Francs',
    id: 'XAF',
  },
  {
    currencyId: 39,
    text: 'XAU - Gold',
    id: 'XAU',
    disabled: true,
  },
  {
    currencyId: 40,
    text: 'ZAR - South African Rand',
    id: 'ZAR',
  },
  {
    currencyId: 41,
    text: 'BHD - Bahrain Dinar',
    id: 'BHD',
  },
  {
    currencyId: 42,
    text: 'CNY - Chinese Yuan Renminbi',
    id: 'CNY',
    disabled: true,
  },
  {
    currencyId: 43,
    text: 'PHP - Philippine Peso',
    id: 'PHP',
  },
  {
    currencyId: 44,
    text: 'UGX - Uganda Shilling',
    id: 'UGX',
  },
  {
    currencyId: 46,
    text: 'KES - Kenyan Shilling',
    id: 'KES',
  },
  {
    currencyId: 47,
    text: 'MOP - Pataca',
    id: 'MOP',
    disabled: true,
  },
  {
    currencyId: 48,
    text: 'CLP - Chilean Peso',
    id: 'CLP',
  },
  {
    currencyId: 49,
    text: 'HUF - Hungarian Forint',
    id: 'HUF',
  },
  {
    currencyId: 50,
    text: 'ISK - Icelandic Krona',
    id: 'ISK',
  },
  {
    currencyId: 51,
    text: 'IDR - Indonesian Rupiah',
    id: 'IDR',
  },
  {
    currencyId: 52,
    text: 'MUR - Mauritian Rupee',
    id: 'MUR',
  },
  {
    currencyId: 53,
    text: 'PEN - Peruvian Sol',
    id: 'PEN',
  },
  {
    currencyId: 54,
    text: 'RUB - Russian Ruble ',
    id: 'RUB',
  },
  {
    currencyId: 55,
    text: 'KRW - South Korean Won',
    id: 'KRW',
  },
  {
    currencyId: 56,
    text: 'TRY - Turkish Lira',
    id: 'TRY',
  },
  {
    currencyId: 57,
    text: 'UYU - Uruguayan Peso',
    id: 'UYU',
  },
  {
    currencyId: 58,
    text: 'ZMK - Zambian Kwacha',
    id: 'ZMK',
  },
  {
    currencyId: 59,
    text: 'MAD  - Moroccan Dirham',
    id: 'MAD',
  },
];

/**
 * Regular expression group to match valid currency codes.
 * This regex captures any of the currency codes defined in the CURRENCY enum.
 * Example matches: "GBP", "USD", etc.
 */
export const CURRENCY_REGEX_GROUP = `(?<currency>${Object.values(CURRENCY).join('|')})`;
export const CURRENCY_REGEX = new RegExp(CURRENCY_REGEX_GROUP);
