 ...
 *
 * This middleware applies the `clean` function to `req.params`, `req.body`, and `req.query`
...
export const xss = (req: Request, _res: Response, next: NextFunction) => {
  try {
    ...
    if (req.body) {
      req.body = clean(req.body as Record<string, unknown>) as Record<string, unknown>;
    }
