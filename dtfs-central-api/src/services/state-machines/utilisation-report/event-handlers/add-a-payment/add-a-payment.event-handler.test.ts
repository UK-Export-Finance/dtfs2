...
describe('handleUtilisationReportAddAPaymentEvent', () => {
  ...
  it(`only updates the report audit fields if the report status is '${UTILISATION_REPORT_STATUS.RECONCILIATION_IN_PROGRESS}'`, async () => {
    // Arrange
    const reconciliationInProgressReport = UtilisationReportEntityMockBuilder.forStatus(UTILISATION_REPORT_STATUS.RECONCILIATION_IN_PROGRESS).build();
  ...
  it(`updates the report audit fields and status if the report status is '${UTILISATION_REPORT_STATUS.PENDING_RECONCILIATION}'`, async () => {
    // Arrange
    const pendingReconciliationReport = UtilisationReportEntityMockBuilder.forStatus(UTILISATION_REPORT_STATUS.PENDING_RECONCILIATION).build();
    ...
    expect(pendingReconciliationReport.status).toEqual(UTILISATION_REPORT_STATUS.RECONCILIATION_IN_PROGRESS);
  });
});
