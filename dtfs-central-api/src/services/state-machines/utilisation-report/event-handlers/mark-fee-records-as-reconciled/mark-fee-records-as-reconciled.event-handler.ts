...
import { DbRequestSource, FEE_RECORD_STATUS, FeeRecordEntity, UTILISATION_REPORT_STATUS, UtilisationReportEntity } from '@ukef/dtfs2-common';
...
export const handleUtilisationReportMarkFeeRecordsAsReconciledEvent = async (
  ...
  try {
    ...
    if (allFeeRecords.every((record) => record.status === FEE_RECORD_STATUS.RECONCILED)) {
      report.updateWithStatus({ status: UTILISATION_REPORT_STATUS.RECONCILIATION_COMPLETED, requestSource });
