...
describe('calculateFixedFeeAdjustment', () => {
  ...
  describe('when the report period starts before the cover start date', () => {
    it('returns the product of the utilisation, interest percentage, bank admin fee percentage (fixed at 0.9) and number of days remaining in the cover period divided by the day count basis and rounds to 2 decimal places', () => {
      // Arrange
      const utilisation = 100000;
      const interestPercentage = 0.05;
      ...
      // Assert
      expect(result).toBe(4536.99); // 100000 * 0.05 * 0.9 * 368 / 365 = 4,536.98630... = 4,536.99 (ROUNDED)
    });
  });
  ...
  describe('when the report period starts after the cover start date', () => {
    it('returns the product of the utilisation, interest percentage, bank admin fee percentage (fixed at 0.9) and number of days remaining in the cover period divided by the day count basis and rounds to 2 decimal places', () => {
      ...
      // Assert
      expect(result).toBe(5326.03); // 100000 * 0.05 * 0.9 * 432 / 365 = 5,326.02739... = 5,326.03 (ROUNDED)
    });
  });
});
