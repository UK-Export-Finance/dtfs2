...
export const validateUtilisationReportPeriodSchedule = (utilisationReportPeriodSchedule: BankReportPeriodSchedule): string | undefined => {
  let validationError: string | undefined;

  if (!utilisationReportPeriodSchedule || !utilisationReportPeriodSchedule.length) {
  ...
  for (let i = 0; i < utilisationReportPeriodSchedule.length; i += 1) {
    if (!utilisationReportPeriodSchedule[i].startMonth || !utilisationReportPeriodSchedule[i].endMonth) {
      validationError = 'Utilisation Report Period Schedule does not have start or end month';
      return validationError;
    };
  };

  const { monthsInSchedule, firstScheduleError } = getMonthsInSchedule(utilisationReportPeriodSchedule);
