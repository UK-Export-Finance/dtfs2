...
describe('reconciliation-summary-item-mapper', () => {
  describe('mapReportToSummaryItem', () => {
    it('sets the total facilities reported to the count of distinct facility ids in the reports fee records', () => {
      // Arrange
      const report = UtilisationReportEntityMockBuilder.forStatus(UTILISATION_REPORT_STATUS.PENDING_RECONCILIATION).build();
    ...
    it('sets the reported fees left to reconcile to zero if the report is reconciled but not all fee records are', () => {
      // Arrange
      const report = UtilisationReportEntityMockBuilder.forStatus(UTILISATION_REPORT_STATUS.RECONCILIATION_COMPLETED).withFeeRecords([]).build();
    ...
    it('does not set the reconciliation summary count fields if the report has not yet been received', () => {
      // Arrange
      const report = UtilisationReportEntityMockBuilder.forStatus(UTILISATION_REPORT_STATUS.REPORT_NOT_RECEIVED).withFeeRecords([]).build();
