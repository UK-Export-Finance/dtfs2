...
describe('get selected fee record details controller helpers', () => {
  ...
  describe('mapToSelectedFeeRecordDetails', () => {
    it('sets bank name', async () => {
      // Act
      const result = await mapToSelectedFeeRecordDetails('123', aReportPeriod(), [aFeeRecord()]);
    ...
    it('maps report period', async () => {
      ...
      // Act
      const result = await mapToSelectedFeeRecordDetails('123', reportPeriod, [aFeeRecord()]);
    ...
    it('maps the fee record facilityId, id and exporter', async () => {
      ...
      const feeRecord = FeeRecordEntityMockBuilder.forReport(UtilisationReportEntityMockBuilder.forStatus('PENDING_RECONCILIATION').build())
        ...
        .build();
      // Act
      const result = await mapToSelectedFeeRecordDetails('123', aReportPeriod(), [feeRecord]);
    ...
    it('calculates total reported payments in payment currency by adding up reported payments of all fee records', async () => {
      // Arrange
      const feeRecordWithDifferingCurrencies = FeeRecordEntityMockBuilder.forReport(
      ...
      const feeRecordWithMatchingCurrencies = FeeRecordEntityMockBuilder.forReport(
        UtilisationReportEntityMockBuilder.forStatus('PENDING_RECONCILIATION').build(),
      )
      ...
      // Act
      const result = await mapToSelectedFeeRecordDetails('123', aReportPeriod(), [feeRecordWithDifferingCurrencies, feeRecordWithMatchingCurrencies]);
