...
export const feeRecordCsvRowToSqlEntity = ({ dataEntry, requestSource, report }: FeeRecordCsvRowToSqlEntityParams): FeeRecordEntity => {
  ...
  const reportedPaymentCurrency = dataEntry[UTILISATION_REPORT_HEADERS.PAYMENT_CURRENCY];
  const reportedAccrualCurrency = dataEntry[UTILISATION_REPORT_HEADERS.TOTAL_FEES_ACCRUED_CURRENCY];

  const paymentCurrency = reportedPaymentCurrency || dataEntry[UTILISATION_REPORT_HEADERS.FEES_PAID_IN_PERIOD_CURRENCY];
  const totalFeesAccruedForThePeriodCurrency = reportedAccrualCurrency || dataEntry[UTILISATION_REPORT_HEADERS.BASE_CURRENCY];
