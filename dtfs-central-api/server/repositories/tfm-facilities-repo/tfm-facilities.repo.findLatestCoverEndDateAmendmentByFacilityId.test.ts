...
describe('TfmFacilitiesRepo', () => {
  describe('findLatestCoverEndDateAmendmentByFacilityId', () => {
    beforeEach(() => {
      jest.resetAllMocks();
    ...
    it(`should call getCollection with ${MONGO_DB_COLLECTIONS.TFM_FACILITIES}`, async () => {
      // Act
      await TfmFacilitiesRepo.findLatestCoverEndDateAmendmentByFacilityId(facilityId);
  ...
  describe('when there is only one completed cover end date amendment', () => {
    it('should return the amendment', async () => {
      ...
      const result = await TfmFacilitiesRepo.findLatestCoverEndDateAmendmentByFacilityId(facilityId);

      expect(result).toEqual(anAcknowledgedPortalAmendment);
    });
  });

  describe('when there are multiple completed cover end date amendments', () => {
  ...
  describe('when there are no completed cover end date amendments', () => {
