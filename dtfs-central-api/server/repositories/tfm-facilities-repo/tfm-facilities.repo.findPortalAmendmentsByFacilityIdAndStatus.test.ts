...
const facilityWithPortalAmendments: TfmFacility = aTfmFacility({ amendments: [aDraftPortalAmendment, anAcknowledgedPortalAmendment] });
const facilityWithMixedAmendments: TfmFacility = aTfmFacility({ amendments: [aReadyForCheckersApprovalPortalAmendment, aTfmAmendment] });

describe('TfmFacilitiesRepo', () => {
  describe('findPortalAmendmentsByFacilityIdAndStatus', () => {
    beforeEach(() => {
      jest.resetAllMocks();
    ...
    it(`should call getCollection with ${MONGO_DB_COLLECTIONS.TFM_FACILITIES}`, async () => {
      // Act
      await TfmFacilitiesRepo.findPortalAmendmentsByFacilityIdAndStatus({ facilityId });
    ...
    it('should call find with the expected parameters', async () => {
      ...
      const expectedFilter = {
        'facilitySnapshot._id': { $eq: new ObjectId(facilityId) },
        'amendments.type': {
    ...
    it('should return the all portal amendments when no status filter is passed in', async () => {
      ...
      // Act
      const result = await TfmFacilitiesRepo.findPortalAmendmentsByFacilityIdAndStatus({ facilityId });
    ...
    it('should return the portal amendments filtered by status when a single status is passed in', async () => {
      ...
      // Act
      const result = await TfmFacilitiesRepo.findPortalAmendmentsByFacilityIdAndStatus({ facilityId, statuses: [DRAFT] });
    ...
    it('should return the portal amendments filtered by status when a multiple statuses are passed in', async () => {
      ...
      // Act
      const result = await TfmFacilitiesRepo.findPortalAmendmentsByFacilityIdAndStatus({
        facilityId,
