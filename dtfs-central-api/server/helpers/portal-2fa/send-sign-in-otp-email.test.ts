...
const aPortalUser = (overrides: Partial<PortalUser> = {}): PortalUser =>
  ({
    _id: 'user-1',
    email: 'test@test.com',
    firstname: 'John',
...
describe('sendSignInOtpEmail', () => {
  ...
  describe('when the user has all required fields', () => {
    it('should call externalApi.sendEmail with the correct template, email address and variables', async () => {
      ...
      expect(sendEmailSpy).toHaveBeenCalledWith(EMAIL_TEMPLATE_IDS.PORTAL_ACCESS_CODE_EMAIL, 'test@test.com', {
        firstName: 'John',
        lastName: 'Doe',
        securityCode: '123456',
      });
  ...
  describe('when externalApi.sendEmail throws a non-Axios error', () => {
    ...
    it('should throw an EmailSendError without an HTTP status', async () => {
      ...
      // Assert
      await expect(sendOtpToUser).rejects.toThrow(new EmailSendError('user-1', 'send failed'));
    });
    ...
    it('should log an error', async () => {
      ...
      // Assert
      await expect(sendOtpToUser).rejects.toThrow();
  ...
  describe('when externalApi.sendEmail throws an Axios error', () => {
    ...
    it('should throw an EmailSendError with the HTTP status', async () => {
      ...
      // Assert
      await expect(sendOtpToUser).rejects.toThrow(new EmailSendError('user-1', 'Bad Gateway', 502));
    });
