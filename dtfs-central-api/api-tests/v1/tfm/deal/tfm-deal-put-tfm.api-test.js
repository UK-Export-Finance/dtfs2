...
describe('/v1/tfm/deal/:id', () => {
  ...
  describe('PUT /v1/tfm/deal/:id', () => {
    ...
    it('404s if updating an unknown id', async () => {
      const { status } = await api.put({ dealUpdate, user: MOCK_TFM_USER}).to('/v1/tfm/deals/61e54e2e532cf2027303e001');
    ...
    it('updates the created deal with correct fields, retaining original dealSnapshot', async () => {
      ...
      const { status } = await api.put({ dealUpdate, user: MOCK_TFM_USER}).to(`/v1/tfm/deals/${dealId}`);
      ...
      expect(dealAfterUpdate.auditDetails).toEqual({
        ...
        lastUpdatedByTfmUserId: MOCK_PORTAL_USER._id,
    ...
    it('updates deal.tfm.lastUpdated on each update', async () => {
      ...
      // first update
      await api.put({ dealUpdate: anUpdate, user: MOCK_TFM_USER}).to(`/v1/tfm/deals/${dealId}`);
    ...
    it('should not add blank activity object to deal.tfm.activities', async () => {
      ...
      await api.put({ dealUpdate: tfmObject, user: MOCK_TFM_USER}).to(`/v1/tfm/deals/${dealId}`);
      ...
      await api.put({ dealUpdate: blankActivity, user: MOCK_TFM_USER}).to(`/v1/tfm/deals/${dealId}`);
    ...
    it('should add correct activity object to tfm in reverse order', async () => {
      ...
      await api.put({ dealUpdate: singleCommentActivity, user: MOCK_TFM_USER}).to(`/v1/tfm/deals/${dealId}`);
      ...
      await api.put({ dealUpdate: secondCommentActivity, user: MOCK_TFM_USER}).to(`/v1/tfm/deals/${dealId}`);
