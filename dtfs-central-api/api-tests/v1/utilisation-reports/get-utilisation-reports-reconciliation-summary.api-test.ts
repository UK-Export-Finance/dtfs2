...
describe(`GET ${BASE_URL}`, () => {
  beforeAll(async () => {
    ...
    await testApi.post(withoutMongoId(MOCK_BANKS.bank1)).to('/v1/bank');
  ...
  it('returns a 200 response with the correct number of associated fee records', async () => {
    // Arrange
    const reportPeriod = getCurrentReportPeriodForBankSchedule(MOCK_BANKS.bank1.utilisationReportPeriodSchedule);
    ...
    const utilisationReport = UtilisationReportEntityMockBuilder.forStatus(RECONCILIATION_IN_PROGRESS)
      .withBankId(MOCK_BANKS.bank1.id)
