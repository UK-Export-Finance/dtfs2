  ...
  beforeEach(() => {
    ...
    cy.wrap(visibleBanks).each((bank) => {
      if (bank.id === BANK_WITH_REPORTS_ID) {
        ...
        const mockPendingReconciliationUtilisationReport = UtilisationReportEntityMockBuilder.forStatus(UTILISATION_REPORT_STATUS.PENDING_RECONCILIATION)
          .withId('1')
          .withBankId(bank.id)
          .withReportPeriod(getReportPeriod(10))
          .build();
        const mockNotReceivedUtilisationReport = UtilisationReportEntityMockBuilder.forStatus(UTILISATION_REPORT_STATUS.REPORT_NOT_RECEIVED)
          .withId('2')
          .withBankId(bank.id)
