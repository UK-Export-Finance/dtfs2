const utilisationReportsPage = {
  ...
  clickMarkReportAsCompletedButton: (submissionMonth) => cy.get(`[data-cy="utilisation-reports-form--${submissionMonth}"]`)
    .within(($form) => {
      cy.wrap($form).get('[data-cy="mark-report-as-completed-button"]').click();
    }),
  clickMarkReportAsNotCompletedButton: (submissionMonth) => cy.get(`[data-cy="utilisation-reports-form--${submissionMonth}"]`)
    .within(($form) => {
