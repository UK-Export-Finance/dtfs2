...
context('Deal cancellation', () => {
  ...
  before(() => {
    ...
    cy.insertOneGefDeal(MOCK_APPLICATION_MIN_DRAFT, BANK1_MAKER1).then((insertedDeal) => {
      ...
      cy.createGefFacilities(dealId, [multipleMockGefFacilities({ facilityEndDateEnabled: true }).unissuedContingentFacility], BANK1_MAKER1).then(
        (createdFacilities) => {
  ...
  describe('effective date in the future', () => {
    ...
    it('Gef MIN deal with unissued facilities is submitted to UKEF, user cancel deal in TFM. Maker unable issue facility on portal', () => {
      //-----------------------------------------------------------------------
