...
import { D_MMMM_YYYY_FORMAT, DD_MMM_YYYY_FORMAT, twoYears, twoDays, today, threeYears } from '@ukef/dtfs2-common/test-helpers';
...
const CHANGE_FACILITY_VALUE_3 = '40000';

context(
  'Amendments - TFM - Correct values should be displayed when multiple amendments with empty string reference numbers are made to a facility, and a tfm amendment with reference number is subsequently made',
  () => {
    let dealId;
    let facilityId;
    let tfmDealPage;
    ...
    before(() => {
      cy.insertOneGefDeal(MOCK_APPLICATION_AIN_DRAFT, BANK1_MAKER1).then((insertedDeal) => {
        ...
        cy.createGefFacilities(dealId, [mockFacility], BANK1_MAKER1).then((createdFacility) => {
          ...
          cy.submitTfmAmendment({ dealId, facilityId, facilityValue: CHANGED_FACILITY_VALUE, coverEndDate: twoDays.date }).then((amendmentId) => {
            cy.updateAmendment(facilityId, amendmentId, { referenceNumber: '' });
          });
          ...
          cy.submitTfmAmendment({ dealId, facilityId, facilityValue: CHANGE_FACILITY_VALUE_2, coverEndDate: twoYears.date }).then((amendmentId) => {
            cy.updateAmendment(facilityId, amendmentId, { referenceNumber: '' });
          });
          ...
          cy.submitTfmAmendment({ dealId, facilityId, facilityValue: CHANGE_FACILITY_VALUE_3, coverEndDate: threeYears.date });
        });
      });
    });
    ...
    it('should display the amended facility value on facility summary list for the tfm facility value amendment', () => {
      const formattedValueWithCurrency = `${CURRENCY.GBP} ${getFormattedMonetaryValue(CHANGE_FACILITY_VALUE_3, true)}`;
      const exposure = CHANGE_FACILITY_VALUE_3 * 0.8; // 80% exposure
    ...
    it('should display the amended coverEndDate and tenor on facility summary list for the tfm cover end date amendment', () => {
      const tenor = differenceInMonths(new Date(threeYears.date), new Date()) + 1; // +1 to include the current month
      const coverEndDateFormatted = format(new Date(threeYears.date), D_MMMM_YYYY_FORMAT);
    ...
    it('should display the updated value and updated cover end date on the All facilities page for the facility', () => {
      const formattedValueWithCurrency = `${CURRENCY.GBP} ${getFormattedMonetaryValue(CHANGE_FACILITY_VALUE_3, true)}`;
      const coverEndDateFormatted = format(new Date(threeYears.date), DD_MMM_YYYY_FORMAT);
