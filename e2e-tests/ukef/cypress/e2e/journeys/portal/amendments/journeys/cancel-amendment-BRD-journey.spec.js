...
import bankReviewDate from '../../../../../../../gef/cypress/e2e/pages/amendments/facility-end-date';
...
context('Amendments - Cancel amendment journey', () => {
  ...
  describe('Bank review date', () => {
    ...
    it(`should navigate back to bank review date when "No, go back" button clicked`, () => {
      const amendmentUrl = `/gef/application-details/${dealId}/facilities/${facilityId}/amendments/${amendmentId}`;
      cy.visit(relative(`${amendmentUrl}/bank-review-date`));
      bankReviewDate.cancelLink().click();
      cy.url().should('eq', relative(`${amendmentUrl}/cancel`));
      cancel.goBackButton().click();
      cy.url().should('eq', relative(`${amendmentUrl}/bank-review-date`));
    });

    it(`should navigate back to bank review date when "Back" link clicked`, () => {
      const amendmentUrl = `/gef/application-details/${dealId}/facilities/${facilityId}/amendments/${amendmentId}`;
      cy.visit(relative(`${amendmentUrl}/bank-review-date`));
      bankReviewDate.cancelLink().click();
      cy.url().should('eq', relative(`${amendmentUrl}/cancel`));
    ...
    it('should cancel amendment from bank review date page', () => {
      const amendmentUrl = `/gef/application-details/${dealId}/facilities/${facilityId}/amendments/${amendmentId}`;
      cy.visit(relative(`${amendmentUrl}/bank-review-date`));
      bankReviewDate.cancelLink().click();
      cy.url().should('eq', relative(`${amendmentUrl}/cancel`));
