...
context(
  'Amendments - TFM - Correct values should be displayed when multiple amendments with empty string reference numbers are made to a facility, and a value tfm amendment with reference number is subsequently made',
  () => {
    let dealId;
    let facilityId;
    let tfmDealPage;
    ...
    before(() => {
      cy.insertOneGefDeal(MOCK_APPLICATION_AIN_DRAFT, BANK1_MAKER1).then((insertedDeal) => {
        ...
        cy.createGefFacilities(dealId, [mockFacility], BANK1_MAKER1).then((createdFacility) => {
          ...
          cy.submitTfmAmendment({ dealId, facilityId, coverEndDate: threeYears.date });
        });
      });
    });
