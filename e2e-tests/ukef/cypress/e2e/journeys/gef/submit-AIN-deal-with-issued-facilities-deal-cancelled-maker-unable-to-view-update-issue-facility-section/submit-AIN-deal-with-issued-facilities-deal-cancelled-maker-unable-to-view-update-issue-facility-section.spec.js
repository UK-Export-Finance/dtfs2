...
context('Portal to TFM deal submission', () => {
  ...
  before(() => {
    ...
    cy.insertOneGefDeal(MOCK_APPLICATION_AIN_DRAFT, BANK1_MAKER1).then((insertedDeal) => {
      ...
      cy.createGefFacilities(dealId, [anIssuedCashFacility()], BANK1_MAKER1).then((createdFacilities) => {
  ...
  it('Gef AIN deal with issued facilities is submitted to UKEF, user cancel deal in TFM. Maker unable to view update issue facility section on portal', () => {
    //---------------------------------------------------------------
    ...
    tfmPages.caseDealPage
      .dealStage()
      .invoke('text')
      .then((text) => {
        expect(text.trim()).to.contain('Cancelled');
      });
