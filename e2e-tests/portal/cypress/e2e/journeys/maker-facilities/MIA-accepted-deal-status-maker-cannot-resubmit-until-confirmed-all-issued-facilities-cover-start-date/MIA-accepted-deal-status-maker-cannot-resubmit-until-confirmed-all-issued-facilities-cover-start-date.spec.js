...
context(
  'Given a deal that has `Accepted` status with Issued, Unissued, Unconditional and Conditional facilities',
  () => {
    let deal;
    let dealId;
    ...
    it('Maker can `Confirm or change start date` for Issued & Unconditional facilities and only resubmit the deal once all Issued & Unconditional facilities have had their start date confirmed', () => {
      ...
      const issuedSubmittedBond = dealFacilities.bonds.find(
        (b) => b.facilityStage === 'Issued' && b.status === 'Submitted',
      );

      const issuedCompletedBond = dealFacilities.bonds.find(
        (b) => b.facilityStage === 'Issued' && b.status !== 'Submitted',
      ); // `Completed` is generated dynamically

      const unissuedBond = dealFacilities.bonds.find((b) => b.facilityStage === 'Unissued');

      const unconditionalSubmittedLoan = dealFacilities.loans.find(
        (l) => l.facilityStage === 'Unconditional' && l.status === 'Submitted',
      );

      const unconditionalCompletedLoan = dealFacilities.loans.find(
        (l) => l.facilityStage === 'Unconditional' && l.status !== 'Submitted',
      ); // `Completed` is generated dynamically
      ...
      const fourthUnconditionalSubmittedLoan = dealFacilities.loans.find(
        (l) =>
          ...
          l.status === 'Submitted' &&
          l._id !== secondUnconditionalSubmittedLoanId,
      );

      const fourthUnconditionalSubmittedLoanId = fourthUnconditionalSubmittedLoan._id;
      const fourthUnconditionalSubmittedLoanRow = pages.contract.loansTransactionsTable.row(
        fourthUnconditionalSubmittedLoanId,
      );
      ...
      cy.url().should(
        'eq',
        relative(`/contract/${dealId}/bond/${issuedSubmittedBondId}/confirm-requested-cover-start-date`),
      );
      ...
      cy.url().should(
        'eq',
        relative(`/contract/${dealId}/bond/${issuedCompletedBondId}/confirm-requested-cover-start-date`),
      );
      ...
      cy.url().should(
        'eq',
        relative(`/contract/${dealId}/loan/${unconditionalSubmittedLoanId}/confirm-requested-cover-start-date`),
      );
      ...
      cy.url().should(
        'eq',
        relative(`/contract/${dealId}/loan/${unconditionalCompletedLoanId}/confirm-requested-cover-start-date`),
      );
      pages.facilityConfirmCoverStartDate.needToChangeCoverStartDateYes().click();
      pages.facilityConfirmCoverStartDate.coverStartDateDay().type(NEW_LOAN_COVER_START_DATE.getDate());
      pages.facilityConfirmCoverStartDate.coverStartDateMonth().type(NEW_LOAN_COVER_START_DATE.getMonth() + 1);
      ...
      secondUnconditionalSubmittedLoanRow
        ...
        .should('contain.text', 'Confirm start date');
      ...
      const expectedIssuedBondDate = new Date(secondIssuedSubmittedBond.requestedCoverStartDate).toLocaleDateString(
        'en-GB',
      );
      ...
      secondUnconditionalSubmittedLoanRow
        .changeOrConfirmCoverStartDateLink()
        .should('contain.text', 'Start date confirmed');
