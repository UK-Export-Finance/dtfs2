  ...
  createTasks: () => {
    ...
    const overridePortalUserSignInTokensByUsername = async ({ username, newSignInTokens }) => {
      const signInTokens = newSignInTokens.map((newSignInToken) => {
        ...
        const salt = generateSalt();
        const hash = generateHash(signInTokenFromLink, salt);
        const saltHex = salt.toString('hex');
        const hashHex = hash.toString('hex');
     ...
     */
    const removeAllUtilisationReportsFromDb = async () => await SqlDbDataSource.createQueryBuilder().from(UtilisationReportEntity).delete().execute();
     ...
     */
    const removeAllPaymentMatchingTolerancesFromDb = async () =>
      await SqlDbDataSource.createQueryBuilder().from(PaymentMatchingToleranceEntity).delete().execute();
