...
import { tomorrow, threeDaysAgo, addDays, oneYear } from '@ukef/dtfs2-common/test-helpers';
...
context('Check activity feed', () => {
  describe('Check activity feed contains MIA->MIN activity and does not contain any issued facilities activities', () => {
    ...
    it('should not contain already issued facility or unissued facilities', () => {
      ...
      applicationActivities.facilityActivityChangedBy(facilities[0].ukefFacilityId).should('not.exist');
      applicationActivities.facilityActivityCheckedBy(facilities[0].ukefFacilityId).should('not.exist');
      applicationActivities.previousStatusTag(facilities[0].ukefFacilityId).should('not.exist');
      applicationActivities.newStatusTag(facilities[0].ukefFacilityId).should('not.exist');
      applicationActivities.facilityActivityLink(facilities[0].ukefFacilityId).should('not.exist');
