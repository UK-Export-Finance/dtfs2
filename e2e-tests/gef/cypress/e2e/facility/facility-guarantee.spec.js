...
context('Facility Guarantee Page', () => {
  ...
  describe('Visiting facility guarantee page', () => {
    ...
    it('hides back button when visiting page with `change` query', () => {
      cy.visit(
        relative(
          `/gef/application-details/${applications[1].id}/facilities/${applications[1].facilities[1].details._id}/facility-guarantee?status=change`,
        ),
      );
    ...
    it('displays errors when the fee type and day count are not selected', () => {
      cy.visit(
        relative(
          `/gef/application-details/${applications[1].id}/facilities/${applications[1].facilities[1].details._id}/facility-guarantee`,
        ),
      );
